...
<div class="hp-listing__image"
     ...
     data-preview="false"
     data-aspect-ratio="<?php echo esc_attr( hivepress()->asset->get_aspect_ratio( 'landscape_small' ) ); ?>"
     data-url="<?php echo esc_url( hivepress()->router->get_url( 'listing_view_page', [ 'listing_id' => $listing->get_id() ] ) ); ?>">
  <?php
  if ( get_option( 'hp_listing_enable_image_preview' ) && $image_count > 1 ) :
    foreach ( $listing->get_images() as $image ) :
      ...
      <?php else : ?>
        <img src="<?php echo esc_url( $image->get_url( 'hp_landscape_small' ) ); ?>"
    ...
    <a href="<?php echo esc_url( hivepress()->router->get_url( 'listing_view_page', [ 'listing_id' => $listing->get_id() ] ) ); ?>">
      <?php if ( $image_count >= 1 ) : ?>
        ...
        <img src="<?php echo esc_url( $listing->get_image__url( 'hp_landscape_small' ) ); ?>"
             alt="<?php echo esc_attr( $listing->get_title() ); ?>"
             loading="lazy">
      ...
      <?php else : ?>
        ...
        <?php
        // Try to use the first category image as fallback.
        $category_image_url = '';
        foreach ( $listing->get_categories() as $category ) {
          $category_image_url = $category->get_image__url( 'hp_landscape_small' );
          if ( $category_image_url ) {
            // We only need the first category with an image.
            break;
          }
        }
        ...
        if ( $category_image_url ) :
          ...
          <img src="<?php echo esc_url( $category_image_url ); ?>"
               ...
               loading="lazy">
        <?php else : ?>
          <!-- Fallback to default placeholder if no category image -->
