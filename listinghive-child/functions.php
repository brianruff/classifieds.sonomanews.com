...
// ===== Sun Config (MU-plugin) helpers =====
// Pricing/limits are stored via the Sun Config MU-plugin (wp-admin editable).
// These helpers prevent hard-coded values in this theme and provide safe fallbacks.
if ( ! function_exists( 'sun_cfg' ) ) {
    function sun_cfg( string $key, $fallback ) {
        if ( function_exists( 'sun_get_config' ) ) {
            $val = sun_get_config( $key );

            // sun_get_config normalizes numerics already, but guard here too.
...
if ( ! function_exists( 'sun_ad_price_base' ) ) {
    function sun_ad_price_base(): int { return (int) sun_cfg( 'ad_price_base', 25 ); }
}
if ( ! function_exists( 'sun_ad_price_over' ) ) {
    function sun_ad_price_over(): int { return (int) sun_cfg( 'ad_price_over', 35 ); }
}
if ( ! function_exists( 'sun_ad_price_threshold' ) ) {
    function sun_ad_price_threshold(): int { return (int) sun_cfg( 'ad_price_threshold', 240 ); }
}
if ( ! function_exists( 'sun_ad_max_characters' ) ) {
    function sun_ad_max_characters(): int { return (int) sun_cfg( 'ad_max_characters', 360 ); }
}
 ...
 * - If listing description <= threshold chars: show ONLY packages with sun_max_chars = threshold
 * - If listing description >  threshold chars: show ONLY packages with sun_max_chars >= max
 *
...
add_action('pre_get_posts', function (WP_Query $q) {
    ...
    $max_chars = sun_ad_max_characters();

    if ($len <= $threshold) {
        // Show ONLY the shorter-ad packages
        $meta_query[] = [
            'key'     => 'sun_max_chars',
            'value'   => $threshold,
            'compare' => '=',
        ...
        // Show ONLY the longer-ad packages
        $meta_query[] = [
            'key'     => 'sun_max_chars',
            'value'   => $max_chars,
            'compare' => '>=',
...
/**
 * PACKAGE FILTERING (HivePress blocks-level, reliable)
 * Some Paid Listings layouts don't use the WP main query in a way that pre_get_posts can intercept.
 * This hook injects the same sun_max_chars meta_query into any blocks that query packages/products
 * on the listing_submit_package_page template.
 */
if ( ! function_exists( 'sun_hppl_filter_package_blocks' ) ) {
    function sun_hppl_filter_package_blocks( array $blocks, array $wanted_meta ): array {
        foreach ( $blocks as $k => $block ) {
            ...
            if ( isset( $block['blocks'] ) && is_array( $block['blocks'] ) ) {
                $block['blocks'] = sun_hppl_filter_package_blocks( $block['blocks'], $wanted_meta );
            }

            // Inject into any query that fetches packages/products.
            if ( isset( $block['query'] ) && is_array( $block['query'] ) ) {
                $pt = $block['query']['post_type'] ?? null;

                $matches = false;
                if ( is_string( $pt ) ) {
                    $matches = in_array( $pt, [ 'hp_listing_package', 'product' ], true );
                } elseif ( is_array( $pt ) ) {
                    $matches = (bool) array_intersect( $pt, [ 'hp_listing_package', 'product' ] );
                }

                if ( $matches ) {
                    if ( empty( $block['query']['meta_query'] ) || ! is_array( $block['query']['meta_query'] ) ) {
                        $block['query']['meta_query'] = [];
                    }

                    $block['query']['meta_query'][] = $wanted_meta;
                }
            }
...
add_filter(
    'hivepress/v1/templates/listing_submit_package_page/blocks',
    function ( $blocks, $template ) {
        ...
        return sun_hppl_filter_package_blocks( (array) $blocks, $wanted_meta );
    },
    1000,
    2
);
...
add_filter('woocommerce_add_to_cart_validation', function ($passed, $product_id, $quantity) {
    ...
    if ($len <= $threshold && $pkg_max !== $threshold) {
        wc_add_notice(
            sprintf('Your ad is %d characters or less. Please choose the %d-character package.', $threshold, $threshold),
            'error'
        );
        return false;
    }

    if ($len > $threshold && $pkg_max < $max_chars) {
        wc_add_notice(
            sprintf('Your ad is over %d characters. Please choose the %d-character package.', $threshold, $max_chars),
            'error'
        );
        return false;
    }
...
function sun_render_max_chars_box($post) {
    ...
    <input
        ...
        step="1"
        style="width:100%;"
        placeholder="e.g. <?php echo esc_attr($threshold); ?> or <?php echo esc_attr($max_chars); ?>"
    />
    <p style="margin-top:6px;color:#555;">
        <?php echo esc_html($threshold); ?> = shorter ad package<br>
        <?php echo esc_html($max_chars); ?> = longer ad package
    </p>
    <?php
}
...
// NOTE: Client-side package hiding was removed.
// was removed. Server-side filtering (pre_get_posts) and add-to-cart validation now
// enforce package selection based strictly on character count and sun_max_chars meta.
...
add_filter(
    ...
    function( $template ) {
        $threshold = sun_ad_price_threshold();
        $max_chars = sun_ad_max_characters();
        $base      = sun_ad_price_base();
        $over      = sun_ad_price_over();

        $content = sprintf(
            '<div class="sun-create-ad-intro" style="font-size:14px">
                <p><b style="color:red">Instructions:</b><br>
...
<b>Instructions:</b> 
 <a href="/wp-content/uploads/2026/02/how-to-place-a-classified-ad.pdf" target="_blank">How to Place a Classified Ad <svg style="width: 14px;
    position: relative;
    top: 1px;
    color: #329be5;" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
  <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
</svg></a> 
<br>
<br>You must <b><a href="#user_register_modal">register</a></b> before signing in:
<br>
...
/**
 * Hide mismatched packages on listing_submit_package_page (front-end safety net).
 * Keeps only packages where sun_max_chars matches the ad length tier.
 */
add_action('wp_footer', function () {
    if (empty($_GET['hp_route']) || $_GET['hp_route'] !== 'listing_submit_package_page') {
        return;
    ...
    $listing_id = function_exists('hppl_resolve_listing_id') ? (int) hppl_resolve_listing_id() : 0;
    if (!$listing_id) return;

    // Use the same length logic everywhere.
    $content = (string) get_post_field('post_content', $listing_id);
    $content = html_entity_decode(wp_strip_all_tags($content), ENT_QUOTES | ENT_HTML5, 'UTF-8');
    $content = preg_replace('/\s+/u', ' ', trim($content));
    $len     = function_exists('mb_strlen') ? mb_strlen($content, 'UTF-8') : strlen($content);

    // Pull config values (from MU plugin if available, else fallbacks).
    $threshold = function_exists('sun_get_config') ? (int) sun_get_config('ad_price_threshold') : 240;
    $max_chars = function_exists('sun_get_config') ? (int) sun_get_config('ad_max_characters')  : 360;

    // Map package_id => sun_max_chars
    ...
    foreach ($package_ids as $pid) {
        $v = (int) get_post_meta($pid, 'sun_max_chars', true);
        if ($v > 0) $map[(string)$pid] = $v;
    }
    ...
    (function () {
      ...
      if (!cfg || !cfg.pkgMap) return;

      var adLen = Number(cfg.adLen || 0);
      var threshold = Number(cfg.threshold || 240);
      var maxChars = Number(cfg.maxChars || 360);

      var isBaseTier = adLen <= threshold;

      // Find all package buttons and remove the entire grid item when mismatched.
      var buttons = document.querySelectorAll('.hp-listing-package__select-button[data-url]');
      buttons.forEach(function(btn) {
        var url = btn.getAttribute('data-url') || '';
        var m = url.match(/[?&]hp_listing_package_id=(\d+)/);
        if (!m) return;

        var pkgId = m[1];
        var pkgMax = Number(cfg.pkgMap[pkgId] || 0);

        // If meta missing, do nothing (leave visible).
        if (!pkgMax) return;

        var keep = true;

        if (isBaseTier) {
          // Only show threshold packages (e.g., 240)
          keep = (pkgMax === threshold);
        } else {
          // Only show higher tier packages (e.g., >= 360)
          keep = (pkgMax >= maxChars);
        }

        if (!keep) {
          var item = btn.closest('.hp-grid__item') || btn.closest('.hp-listing-package');
          if (item) item.remove();
        }
      });
    })();
...
// Change the maximum number of images per listing.
add_filter(
  'hivepress/v1/models/listing',
  function( $model ) {
    if ( isset( $model['fields']['images'] ) ) {
      $model['fields']['images']['max_items'] = 10; // <-- set your limit
    }
    return $model;
  },
  1000
